...
std::string getAnsiColor(const std::string& code) {
    if (code == "CK") return "\033[38;5;130m"; // Coklat
    if (code == "BM") return "\033[96m";       // Biru Muda
    if (code == "PK") return "\033[95m";       // Pink
    if (code == "AB") return "\033[90m";       // Abu-abu
    if (code == "OR") return "\033[38;5;208m"; // Orange
    if (code == "MR") return "\033[91m";       // Merah
    if (code == "KN") return "\033[93m";       // Kuning
    if (code == "HJ") return "\033[92m";       // Hijau
    if (code == "BT") return "\033[94m";       // Biru Tua
    if (code == "DF") return "\033[97m";       // Putih/Default
...
std::pair<std::string, std::string> getCellStrings(int pos, const std::vector<std::unique_ptr<Petak>>& board, const std::vector<Player>& players) {
    ...
    if (line1.length() > 12) line1 = line1.substr(0, 12);
    ...
    if (line2.length() > 12) line2 = line2.substr(0, 12);
...
void BoardView::printBoard(const std::vector<std::unique_ptr<Petak>>& board, const std::vector<Player>& players, int turnCount) {
    ...
    // Lebar tengah (tanpa 2 kotak di ujung): 144 - 1 (kiri) - 12 (sel) - 1 (kanan) - 12 (sel) - 2 (batas) = 116 spasi
    std::vector<std::string> centerStr(18, std::string(116, ' '));
    auto putText = [&](int row, const std::string& txt) {
        if (row < 0 || row >= static_cast<int>(centerStr.size())) return;
